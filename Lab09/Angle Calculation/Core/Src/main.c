...
void calibrate_accel_y(void) {
  ...
  for (int i = 0; i < num_readings; i++) {
      ...
      if (HAL_I2C_Mem_Read(&hi2c1, IMU_ADDRESS_READ, Y_OUT_L, I2C_MEMADD_SIZE_8BIT, &yl, 1, HAL_MAX_DELAY) != HAL_OK) {
        HAL_UART_Transmit(&huart1, (uint8_t*)"\t Error! \r\n", 11, HAL_MAX_DELAY);
      };
      if (HAL_I2C_Mem_Read(&hi2c1, IMU_ADDRESS_READ, Y_OUT_H, I2C_MEMADD_SIZE_8BIT, &yh, 1, HAL_MAX_DELAY) != HAL_OK) {
        HAL_UART_Transmit(&huart1, (uint8_t*)"\t Error! \r\n", 11, HAL_MAX_DELAY);
      };
...
void init_acc(){
  ...
  if (HAL_I2C_Mem_Write(&hi2c1, IMU_ADDRESS_WRITE, 0x20, I2C_MEMADD_SIZE_8BIT, &ctrl1, 1, HAL_MAX_DELAY) != HAL_OK) {
    HAL_UART_Transmit(&huart1, (uint8_t*)"\t Error! \r\n", 11, HAL_MAX_DELAY);
  };

  if (HAL_I2C_Mem_Write(&hi2c1, IMU_ADDRESS_WRITE, 0x23, I2C_MEMADD_SIZE_8BIT, &ctrl4, 1, HAL_MAX_DELAY) != HAL_OK) {
    HAL_UART_Transmit(&huart1, (uint8_t*)"\t Error! \r\n", 11, HAL_MAX_DELAY);
  };

}

void print_angle(int16_t gyro_y) { 

  char msg[128];

  float final_accel = read_accel_y() - y_acc_offset;
  float gyro_filtered = gyro_y - gyro_offset;
